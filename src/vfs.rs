...
impl Default for OpfsSAHPoolCfg {
    fn default() -> Self {
        Self {
            vfs_name: "enc-sahpool".into(), // ENC: distinct default so it never shadows plain sahpool
            directory: ".enc-sahpool".into(),
            clear_on_init: false,
            initial_capacity: 6,
        }
    }
...
/// Register the encrypting `enc-sahpool` VFS with the injected DEK and return a management tool.
///
