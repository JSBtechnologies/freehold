...
//!  header = magic "ENCENV2\0"(8) | version(1)=2 | slot_count(1) | reserved(2) | env_salt(16)
//!  slot   = kek_id(1) | kind(1) | nonce(24) | wrapped_dek_ct(32) | tag(16)          (74 bytes)
//!  aad    = "enc-sahpool-envelope-v2" | kek_id | kind
//!
//!  kind 0 (passkey):  KEK = HKDF-SHA256(prf_output, info="enc-sahpool-kek-v1")
...
const KEK_INFO: &[u8] = b"enc-sahpool-kek-v1";
const WRAP_AAD_PREFIX: &[u8] = b"enc-sahpool-envelope-v2";
...
fn parse(blob: &[u8]) -> Result<usize, EnvelopeError> {
    if blob.len() < HEADER_LEN || &blob[..8] != MAGIC.as_slice() || blob[8] != VERSION {
        return Err(EnvelopeError::Format);
    }
    let slot_count = blob[9] as usize;
    if blob.len() != HEADER_LEN + slot_count * SLOT_LEN {
        return Err(EnvelopeError::Format);
    }
    Ok(slot_count)
}
...
/// Create a new envelope with a single passkey slot (kek_id 0). Generates the per-envelope salt.
pub fn create_envelope(dek: &[u8; DEK_LEN], prf_output: &[u8]) -> Result<Vec<u8>, EnvelopeError> {
    ...
    blob[8] = VERSION;
    blob[9] = 0; // slot_count, bumped below
    getrandom::getrandom(&mut blob[SALT_OFF..SALT_OFF + SALT_LEN]).map_err(|_| EnvelopeError::Rng)?;
    add_passkey_slot(&blob, dek, prf_output)
}

/// Append a passkey slot wrapping the SAME `dek` under the KEK derived from `prf_output`.
pub fn add_passkey_slot(blob: &[u8], dek: &[u8; DEK_LEN], prf_output: &[u8]) -> Result<Vec<u8>, EnvelopeError> {
    let n = parse(blob)?;
    let kek = kek_from_prf(prf_output);
    let slot = wrap_slot(dek, &kek, next_kek_id(blob, n), KIND_PASSKEY)?;
    Ok(append_slot(blob, n, &slot))
}
...
fn append_slot(blob: &[u8], slot_count: usize, slot: &[u8]) -> Vec<u8> {
    let mut out = blob.to_vec();
    out[9] = (slot_count + 1) as u8;
    out.extend_from_slice(slot);
    out
}

/// Remove the slot with `kek_id`. Refuses to remove the last remaining slot (would orphan the DEK).
pub fn remove_slot(blob: &[u8], kek_id: u8) -> Result<Vec<u8>, EnvelopeError> {
    let n = parse(blob)?;
    if n <= 1 {
        return Err(EnvelopeError::Format);
    }
    let mut out = blob[..HEADER_LEN].to_vec();
    let mut kept = 0usize;
    for i in 0..n {
        let at = HEADER_LEN + i * SLOT_LEN;
        if blob[at] != kek_id {
            out.extend_from_slice(&blob[at..at + SLOT_LEN]);
            kept += 1;
        }
    }
    if kept == n {
...
/// Try `kek` against every slot; return the DEK from the first that authenticates.
