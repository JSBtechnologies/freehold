# enc-sahpool — build notes & honest §17 ledger
...
per-DB block key      = K_db = HKDF-SHA256(DEK, "enc-sahpool/vfs-db-v1\0" ‖ db_uuid)  (§17.E)
pool-domain key       = HKDF(DEK, "enc-sahpool-v1")        (files with no owning DB, e.g. temp)
anchor key            = HKDF(DEK, "enc-sahpool-anchor-v1")
